...
function () {
  function Seeder(app, opts) {
    ...
    this.app = app;
    this.opts = opts;
    this.compiler = new _compiler["default"](opts.generators);
  }
  ...
  _createClass(Seeder, [{
    ...
    value: function seedApp() {
      ...
      return new Promise(function (resolve, reject) {
        ...
        debug("Running ".concat(promises.length, " seeder(s)..."));
        return Promise.all(promises).then(function (seeded) {
          debug("Created ".concat(seeded.length, " total items:"), seeded);
    ...
    value: function seed(cfg) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        debug("Params seeding '".concat(cfg.path, "':"), params);
        debug("Param randomize: ".concat(randomize));
        debug("Creating ".concat(count, " instance(s)")); // Delete from service, if necessary

        var shouldDelete = _this2.opts["delete"] !== false && cfg["delete"] !== false;

        if (!shouldDelete) {
          debug("Not deleting any items from ".concat(cfg.path, "."));
        }

        var deletePromise = shouldDelete ? service.remove(null, params) : Promise.resolve([]);
        return deletePromise.then(function (deleted) {
          debug("Deleted from '".concat(cfg.path, ":'"), deleted);
              ...
              return service.create(compiled, params).then(function (created) {
                ...
                if (typeof cfg.callback !== 'function') {
                  ...
                  return cfg.callback(created, _this2.seed.bind(_this2)).then(function (result) {
                    debug("Result of callback on '".concat(cfg.path, "':"), result);
                    return resolve(created);
                  })["catch"](reject);
                }
              })["catch"](reject);
          ...
          if (cfg.template && cfg.disabled !== true) {
            ...
            if (randomize) {
              for (var _i = 0; _i < count; _i++) {
                ...
                debug("Picked random template index ".concat(idx));
          ...
          if (!promises.length) {
            debug("Seeder disabled for ".concat(cfg.path, ", not modifying database."));
            return resolve([]);
          } else {
